...
import type { Currency, InternalBetResponse } from 'shared';
...
function verifyHmac(req: Request, res: Response, next: () => void): void {
...
const playerLocks = new Map<string, Mutex>();

function getPlayerLock(playerId: string): Mutex {
...
// ---------------------------------------------------------------------------
router.post('/wallet/bet', verifyHmac, async (req: Request, res: Response) => {
  ...
  try {
    const result = await getPlayerLock(playerId).runExclusive(async () => {
      ...
      const responseItems: InternalBetResponse['bets'] = [];
      ...
      return { bets: responseItems } as InternalBetResponse;
    });
...
// ---------------------------------------------------------------------------
router.get('/wallet/balance', verifyHmac, (req: Request, res: Response) => {
  ...
  const currency = (req.query.currency as Currency) ?? 'FUN';
